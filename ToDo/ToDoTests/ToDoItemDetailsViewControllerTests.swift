...
class ToDoItemDetailsViewControllerTests: XCTestCase {
    ...
    func test_sendingButtonAction_shouldCheckItemAndDisableDoneButton(){
        let toDoItem = ToDoItem(title: "dummy title")
        sut.toDoItem = toDoItem
        let storeMock = ToDoItemStoreProtocolMock()
        sut.toDoItemStore = storeMock
    ...
    func test_settingToDoItem_shouldHideMapView_whenNoCoordinates() {
        let toDoItem = ToDoItem(
            title: "dummy title",
            location: Location(name: "dummy location")
        )
        sut.toDoItem = toDoItem
